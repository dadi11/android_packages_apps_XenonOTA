...
import com.xenonota.tasks.Downloader;
...
public class Fragment_OTA extends Fragment implements WaitDialogFragment.OTADialogListener {
    ...
    private String DOWNLOADER_ID = "ROM_DOWNLOAD";
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.recovery:
                ReboottoRecovery();
                return true;
            case R.id.clear_downloads_ota:{
                ClearDownloads();
                return true;
            }
            case R.id.magisk:{
                MagiskDownloadTask magiskTask = MagiskDownloadTask.getInstance(false,this);
                if (!magiskTask.getStatus().equals(AsyncTask.Status.RUNNING)) {
                    magiskTask.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, getActivity());
                }
                return true;
            }
        }
    ...
    private void ClearDownloads(){
        ...
        clear.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                File BaseDir = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + "XenonOTA");
    ...
    private void downloadROM(){
        if(updateAvailable == true && ota_data != null){
            Downloader downloader = new Downloader(MainActivity.getActivity(), ota_data.getROMURL());
            downloader.setDownloadTaskListener(new Downloader.DownloadTask() {
                ...
                public void onDownloadError(String ID, String Error) {
                    AlertDialog.Builder builder;
                    builder = new AlertDialog.Builder(getContext(),R.style.Theme_AppCompat_Light_Dialog_Alert);
                    String errMsg = getString(R.string.download_interrupted_msg) + "\n\n" + Error;
