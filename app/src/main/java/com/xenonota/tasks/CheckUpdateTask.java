...
public class CheckUpdateTask extends AsyncTask<Context, Void, OTADevice> {
    ...
    protected OTADevice doInBackground(Context... params) {
        ...
        String official_url = OTAConfig.getInstance(mContext).getOfficialOtaUrl();
        String experimental_url = OTAConfig.getInstance(mContext).getExperimentalOtaUrl();

        String deviceName = OTAUtils.getDeviceName(mContext);
        OTAUtils.logInfo("deviceName: " + deviceName);
        if (!deviceName.isEmpty()) {
            official = fetchURL(official_url,deviceName);
            if(AppConfig.getPreferredType(mContext)=="Experimental"){experimental = fetchURL(experimental_url,deviceName);}
